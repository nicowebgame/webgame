...
(function(){
  ...
  // input debounce
  let lastMoveTime = 0;
  // allow configurable cooldown (will be loaded from localStorage)
  let moveCooldownMsConfig = parseInt(localStorage.getItem('runner3d_moveCooldown') || '160', 10) || 160;
  // language setting
  let settingsBtn = null;
  let settingsModal = null;
  const supportedLangs = [
    ['en','English'], ['es','Español'], ['pt','Português'], ['zh','中文'], ['hi','हिन्दी'],
    ['ar','العربية'], ['ru','Русский'], ['ja','日本語'], ['de','Deutsch'], ['fr','Français']
  ];
  let currentLang = 'en';
  const i18n = {
    en: {menuText: 'Use Left/Right to change lane. Dodge obstacles!', signIn:'Sign in', register:'Register', guest:'Continue as Guest', play:'Play', restart:'Restart', pause:'Pause', cont:'Continue', settings:'Settings', language:'Language', close:'Close', scoreLabel:'Score', bestLabel:'Best', leadersHeader:'Top Players', signed:'Signed:'},
    es: {menuText: 'Usa Izquierda/Derecha para cambiar de carril. ¡Evita los obstáculos!', signIn:'Entrar', register:'Registrar', guest:'Continuar como invitado', play:'Jugar', restart:'Reiniciar'},
    pt: {menuText: 'Use Esquerda/Direita para mudar de faixa. Desvie dos obstáculos!', signIn:'Entrar', register:'Registrar', guest:'Continuar como convidado', play:'Jogar', restart:'Reiniciar'},
    zh: {menuText: '使用左右切换车道。躲避障碍物！', signIn:'登录', register:'注册', guest:'以访客身份继续', play:'开始', restart:'重新开始'},
    hi: {menuText: 'लेन बदलने के लिए बाएं/दाएं का उपयोग करें। बाधाओं से बचें!', signIn:'साइन इन', register:'रजिस्टर', guest:'अतिथि के रूप में जारी रखें', play:'खेलें', restart:'पुनरारंभ'},
    ar: {menuText: 'استخدم اليسار/اليمين لتغيير المسار. تجنب العوائق!', signIn:'تسجيل الدخول', register:'تسجيل', guest:'متابعة كزائر', play:'تشغيل', restart:'إعادة تشغيل'},
    ru: {menuText: 'Используйте влево/вправо для смены полосы. Избегайте препятствий!', signIn:'Войти', register:'Регистрация', guest:'Продолжить как гость', play:'Играть', restart:'Перезапустить'},
    ja: {menuText: '左右でレーンを変更します。障害物を避けてください！', signIn:'サインイン', register:'登録', guest:'ゲストで続行', play:'再生', restart:'再開'},
    de: {menuText: 'Verwende Links/Rechts, um die Spur zu wechseln. Hindernissen ausweichen!', signIn:'Anmelden', register:'Registrieren', guest:'Als Gast fortfahren', play:'Spielen', restart:'Neustart'},
    fr: {menuText: 'Utilisez Gauche/Droite pour changer de voie. Évitez les obstacles !', signIn:'Se connecter', register:'S’inscrire', guest:'Continuer en invité', play:'Jouer', restart:'Redémarrer'}
  };
  ...
  function applyLanguage(lang){
    ...
    currentLang = lang; localStorage.setItem('runner3d_lang', lang);
    try{ const it = i18n[lang]; if(document.getElementById('menuText')) document.getElementById('menuText').textContent = it.menuText; }catch(e){}
    try{
      if(menuSignBtn) menuSignBtn.textContent = i18n[lang].signIn;
      if(menuRegisterBtn) menuRegisterBtn.textContent = i18n[lang].register;
      if(menuGuestBtn) menuGuestBtn.textContent = i18n[lang].guest;
      const mp = document.getElementById('menuPlayBtn'); if(mp) mp.textContent = i18n[lang].play;
      const mh = document.getElementById('menuHowBtn'); if(mh) mh.textContent = i18n[lang].restart || i18n[lang].play;
      if(pauseBtn) pauseBtn.textContent = i18n[lang].pause;
      if(settingsBtn) settingsBtn.textContent = i18n[lang].settings;
      const topHeader = document.querySelector('#leaderboard h4'); if(topHeader) topHeader.textContent = i18n[lang].leadersHeader || topHeader.textContent;
      const scoreLabelEl = document.querySelector('#hud'); if(scoreLabelEl) scoreLabelEl.innerHTML = (i18n[lang].scoreLabel || 'Score') + ': <span id="score">' + Math.floor(score||0) + '</span>';
      const bestBox = document.getElementById('bestHud'); if(bestBox) bestBox.innerHTML = (i18n[lang].bestLabel || 'Best') + ': <span id="bestValue">' + (bestHudEl?bestHudEl.textContent:0) + '</span>';
      // paused overlay text
      const po = document.getElementById('pauseOverlay'); if(po) po.textContent = i18n[lang].pause || 'Paused';
    }catch(e){}
  }
  ...
  function moveLeft(){ const now = Date.now(); if(now - lastMoveTime < moveCooldownMsConfig) return; lastMoveTime = now; if(!running) return; if(currentLane>0){ currentLane--; targetX = lanes[currentLane]; }}
  function moveRight(){ const now = Date.now(); if(now - lastMoveTime < moveCooldownMsConfig) return; lastMoveTime = now; if(!running) return; if(currentLane<2){ currentLane++; targetX = lanes[currentLane]; }}
...
/* Runner3D - simple endless runner using three.js
   Controls: ArrowLeft / ArrowRight or A/D, touch left/right halves to change lane
*/
(function(){
  const container = document.getElementById('gameContainer');
  const scoreEl = document.getElementById('score');
  ...
  let loader = null;
  let bullets = [];
  const bulletSpeed = 120; // world units per second
  const bulletDmg = 1;
  // speedometer mapping
  const speedoMin = 6;
  const speedoMax = 36;
  ...
  let targetX = lanes[currentLane];
  // user system
  let users = {}; // username -> {pass, best}
  let currentUser = null;
  let specialAuto = false;
  let specialLastChange = 0;
  const specialMinChangeInterval = 0.18; // seconds between lane changes for AI
  const specialLookaheadZ = 260; // how far ahead (in world z) AI considers
  // audio / music
  let audioCtx = null;
  let musicGain = null;
  let musicFilter = null;
  ...
  let lfoGain = null;
  let musicBeatAcc = 0;
  let musicBeatInterval = 0.45; // seconds between subtle pulses
  const baseFreq1 = 110;
  const baseFreq2 = 220;
  // three.js snow particles
  let snowPoints = null;
  let snowVel = null;
  const SNOW_COUNT = 420;
  // houses
  ...
  function initThree(){
    ...
    // load textures from zm/ (pre-bundled in repo)
    loader = new THREE.TextureLoader();
    const zmPaths = ['zm/plantzombie.png','zm/stevezombie.png','zm/tralalelozombie.jpg','zm/sahurzombie.jpg'];
    zmPaths.forEach(p=>{
      loader.load(p,
    ...
    // player -> replace box with a more human-like animated runner
    function createRunner(){
      const g = new THREE.Group();
      const torsoMat = new THREE.MeshStandardMaterial({color:0xffd166, metalness:0.12, roughness:0.5});
      const limbMat = new THREE.MeshStandardMaterial({color:0x2b2b2b, metalness:0.05, roughness:0.9});
      ...
      // torso (slightly tapered to suggest chest/abs) — reduced size
      const torso = new THREE.Mesh(new THREE.BoxGeometry(0.75,1.0,0.75), torsoMat);
      torso.position.set(0,0.9,0);
      g.add(torso);
      ...
      const leftLeg = new THREE.Mesh(new THREE.BoxGeometry(0.28,0.9,0.28), limbMat);
      leftLeg.position.set(-0.23,0.35,0);
      leftLeg.geometry.translate(0,-0.45,0); // pivot at hip
      const rightLeg = new THREE.Mesh(new THREE.BoxGeometry(0.28,0.9,0.28), limbMat);
      rightLeg.position.set(0.23,0.35,0);
      rightLeg.geometry.translate(0,-0.45,0);
      g.add(leftLeg); g.add(rightLeg);

      // arms: upper + lower + hand, set pivots for shoulder/elbow
      const upperArmGeom = new THREE.BoxGeometry(0.22,0.5,0.22);
      upperArmGeom.translate(0, -0.25, 0); // pivot at shoulder
      const lowerArmGeom = new THREE.BoxGeometry(0.18,0.48,0.18);
      lowerArmGeom.translate(0, -0.24, 0); // pivot at elbow

      const leftUpper = new THREE.Mesh(upperArmGeom, limbMat);
      leftUpper.position.set(-0.6,1.05,0);
      const leftLower = new THREE.Mesh(lowerArmGeom, limbMat);
      leftLower.position.set(0, -0.46, 0); // relative to upper arm after parenting
      const leftHand = new THREE.Mesh(new THREE.BoxGeometry(0.12,0.12,0.2), skinMat);
      leftHand.position.set(0, -0.46, 0.06);
      ...
      // attach lower arm and hand to upper arm for hierarchical rotation
      leftUpper.add(leftLower); leftLower.add(leftHand);
      rightUpper.add(rightLower); rightLower.add(rightHand);
      g.add(leftUpper); g.add(rightUpper);

      // small head (more rounded feel)
      const head = new THREE.Mesh(new THREE.BoxGeometry(0.46,0.52,0.46), skinMat);
      head.position.set(0,1.55,0);
      g.add(head);

      // optional bicep/shoulder suggestion: small sphere attached to upper arm
      const bicep = new THREE.SphereGeometry(0.11,8,8);
      const biL = new THREE.Mesh(bicep, limbMat); biL.position.set(0, -0.12, 0); leftUpper.add(biL);
      const biR = new THREE.Mesh(bicep.clone(), limbMat); biR.position.set(0, -0.12, 0); rightUpper.add(biR);
    ...
    playerMesh.position.set(targetX, 0.7, 3.2);
    scene.add(playerMesh);
    // create pause/continue button and overlay in top-right of game container
    try{
      pauseBtn = document.createElement('button');
      pauseBtn.id = 'pauseBtn'; pauseBtn.textContent = i18n[currentLang]?.pause || 'Pause';
      // append into container so CSS absolute positioning is relative to it
      container.appendChild(pauseBtn);
      // overlay (centered) shown on pause
      const overlay = document.createElement('div'); overlay.id = 'pauseOverlayWrap'; overlay.style.position = 'absolute'; overlay.style.inset = '0'; overlay.style.display = 'none'; overlay.style.alignItems = 'center'; overlay.style.justifyContent = 'center'; overlay.style.zIndex = 1600;
      const pText = document.createElement('div'); pText.id = 'pauseOverlay'; pText.textContent = i18n[currentLang]?.pause || 'Paused'; pText.style.background = 'rgba(2,6,23,0.6)'; pText.style.padding = '18px 28px'; pText.style.borderRadius = '12px'; pText.style.color = '#e6eef8'; pText.style.fontSize = '20px';
      overlay.appendChild(pText); container.appendChild(overlay);
      pauseBtn.addEventListener('click', async ()=>{
        if(!running){ // resume
          running = true; paused = false; try{ clock.start(); }catch(e){}; try{ if(audioCtx && audioCtx.state==='suspended') await audioCtx.resume(); }catch(e){}; pauseBtn.textContent = i18n[currentLang]?.pause || 'Pause'; overlay.style.display = 'none'; animate();
        } else { // pause
          running = false; paused = true; try{ clock.stop(); }catch(e){}; try{ if(audioCtx && audioCtx.state==='running') await audioCtx.suspend(); }catch(e){}; pauseBtn.textContent = i18n[currentLang]?.cont || 'Continue'; overlay.style.display = 'flex'; }
      });
    }catch(e){}

    // add houses along sides
    spawnHouses();

    // initialize continuous snow particles
    initSnow();

    // no weapon mesh — player is just a box

    window.addEventListener('resize', onResize);
  }

  function createHouse(side, z){
    const sideX = 8.5; // distance from center
    const colors = [0xff6b6b,0xffd166,0x6be7b0,0x8ec6ff,0xd6a2ff,0xffb3b3];
    const g = new THREE.Group();
    const bx = side * (sideX + (Math.random()*2 -1));
    const height = 1.4 + Math.random()*1.6;
  ...
  function updateHouses(dt){
    ...
    for(const h of housesGroup.children){
      h.position.z += moveZ;
      if(h.position.z > 6){ // passed player
        remove.push(h);
      }
    }
    for(const r of remove){
      // recycle: remove and create new ahead
      const side = Math.sign(r.position.x) || 1;
      const newZ = -700 + Math.random()*-80; // far ahead
      disposeHouse(r);
      housesGroup.remove(r);
      const nh = createHouse(side, newZ);
      housesGroup.add(nh);
  ...
  function initSnow(){
    ...
    snowVel = new Float32Array(SNOW_COUNT);
    for(let i=0;i<SNOW_COUNT;i++){
      const x = (Math.random()*2 -1) * 14; // spread across and beyond road
      const y = 6 + Math.random()*28; // height
      const z = Math.random()*-420 + 40; // in front and far
      positions[i*3] = x; positions[i*3+1] = y; positions[i*3+2] = z;
      snowVel[i] = 0.4 + Math.random()*1.2;
    }
    geom.setAttribute('position', new THREE.BufferAttribute(positions, 3));
    const sprite = null; // use simple points
    const mat = new THREE.PointsMaterial({color:0xffffff, size:0.12, transparent:true, opacity:0.9});
    snowPoints = new THREE.Points(geom, mat);
    snowPoints.frustumCulled = false;
    scene.add(snowPoints);
  ...
  function updateSnow(dt){
    ...
    const pos = snowPoints.geometry.attributes.position.array;
    for(let i=0;i<SNOW_COUNT;i++){
      pos[i*3+1] -= snowVel[i] * dt * 14; // fall speed
      pos[i*3] += Math.sin((pos[i*3+2] + pos[i*3+1]) * 0.01) * 0.02; // drift
      if(pos[i*3+1] < -1){
        pos[i*3+1] = 10 + Math.random()*24;
        pos[i*3] = (Math.random()*2 -1) * 14;
        pos[i*3+2] = Math.random()*-420 + 40;
  ...
  function updateRunner(dt){
    ...
    const norm = clamp((speed - speedoMin) / (speedoMax - speedoMin), 0, 1);
    // cycle rate increases with speed
    const rate = 6 + norm * 18; // base cadence
    ud.runCycle += dt * rate * Math.PI * 2;
    // leg swing amplitude increases with speed but clamped
    const amp = 0.6 + norm * 0.9; // radians
    const lrot = Math.sin(ud.runCycle) * amp;
    const rrot = Math.sin(ud.runCycle + Math.PI) * amp;
    if(ud.leftLeg) ud.leftLeg.rotation.x = lrot;
    if(ud.rightLeg) ud.rightLeg.rotation.x = rrot;
    // torso bob and subtle muscle pulse
    if(ud.torso){
      const bob = Math.abs(Math.sin(ud.runCycle)) * (0.04 + norm * 0.06);
      ud.torso.position.y = 0.95 + bob;
      // pulse scale on X/Z to simulate chest/abs flexing
      const pulse = 1 + Math.abs(Math.sin(ud.runCycle*1.5)) * (0.01 + norm * 0.03);
      ud.torso.scale.x = pulse; ud.torso.scale.z = pulse;
      // slight forward lean at high speed
      ud.torso.rotation.x = -norm * 0.10;
    }
    // head tiny counter-rotation for inertia
    if(ud.head) ud.head.rotation.x = Math.sin(ud.runCycle*0.9) * 0.02 * norm;
    // arm swing opposite to legs
    if(ud.leftUpper && ud.rightUpper){
      const swingAmp = 0.9 + norm * 0.6;
      const lArmRot = Math.sin(ud.runCycle + Math.PI) * swingAmp * 0.9; // opposite leg
      const rArmRot = Math.sin(ud.runCycle) * swingAmp * 0.9;
      ud.leftUpper.rotation.x = lArmRot * 0.9;
      ud.rightUpper.rotation.x = rArmRot * 0.9;
      // forearm follows with delayed bend
      ud.leftLower.rotation.x = Math.max(-1.2, Math.min(0.2, -lArmRot * 0.45));
      ud.rightLower.rotation.x = Math.max(-1.2, Math.min(0.2, -rArmRot * 0.45));
      // slight outward sway
      ud.leftUpper.rotation.z = Math.sin(ud.runCycle*0.7) * 0.06;
  ...
  // --- user/account functions (localStorage) ---
  function loadUsers(){
    const raw = localStorage.getItem('runner3d_users');
    if(raw){
      try{
        // stored as hex of base64(JSON)
        const b64 = hexToStr(raw);
        const txt = atob(b64);
        users = JSON.parse(txt);
      }catch(e){ users = {}; }
    }
    // seed special account if missing (store hashed password)
    if(!users['nicomyw']){
      users['nicomyw'] = {pass: '91584f3215c4e40e02d699bb03010368955a6d6642dc87b2f378f5e6645b1a9f', best: 0};
      saveUsers();
    }
    // load saved credentials (remember me) - we only pre-fill username or try hash-login
    const saved = localStorage.getItem('runner3d_saved');
    if(saved){
      try{
        const s = JSON.parse(saved);
        if(s.user) authUser.value = s.user;
        if(s.passHash){ if(rememberMe) rememberMe.checked = true; /* attempt login by hash */ if(users[s.user] && users[s.user].pass === s.passHash){ currentUser = s.user; specialAuto = (s.user==='nicomyw'); updateUserUI(); authModal.style.display='none'; } }
      }catch(e){}
    }
  }
  ...
  function ensureAudio(){
    ...
    musicFilter.frequency.value = 900;
    musicFilter.Q.value = 0.8;
    // create pad gain and connect chain: filter -> padGain -> master gain -> destination
    padGain = audioCtx.createGain(); padGain.gain.value = 0.85 * musicVolume;
    musicFilter.connect(padGain);
    padGain.connect(musicGain);
    musicGain.connect(audioCtx.destination);
    // two oscillators layered for simple ambient pad
    musicOsc1 = audioCtx.createOscillator(); musicOsc1.type = 'sine'; musicOsc1.frequency.value = 110;
    musicOsc2 = audioCtx.createOscillator(); musicOsc2.type = 'sawtooth'; musicOsc2.frequency.value = 220;
    musicOsc1.connect(musicFilter);
    musicOsc2.connect(musicFilter);
    // LFO to modulate filter cutoff for movement
    musicLFO = audioCtx.createOscillator(); musicLFO.type = 'sine'; musicLFO.frequency.value = 0.24;
    lfoGain = audioCtx.createGain(); lfoGain.gain.value = 300; // modulation depth in Hz
    musicLFO.connect(lfoGain);
    lfoGain.connect(musicFilter.frequency);
    musicLFO.start();
    musicOsc1.start(); musicOsc2.start();
  ...
  function startMusic(){
    try{
      ...
      musicGain.gain.setValueAtTime(musicGain.gain.value || 0, now);
      musicGain.gain.linearRampToValueAtTime(musicVolume * 0.9, now + 0.6);
      // ensure padGain follows volume baseline
      if(padGain) padGain.gain.setTargetAtTime(0.85 * musicVolume, now, 0.1);
      musicMuted = false; localStorage.setItem('runner3d_musicMuted','0');
      updateAudioUI();
    }catch(e){/* ignore */}
  ...
  // resume or init audio on first user gesture (autoplay rules)
  function resumeOnInteraction(){ document.addEventListener('pointerdown', ()=>{
    if(!audioCtx) ensureAudio();
    if(audioCtx && audioCtx.state === 'suspended') audioCtx.resume().catch(()=>{});
  ...
  // legacy synchronous wrappers (not used by UI handlers anymore)
  function registerUser(name, pass){ console.warn('registerUser synchronous is deprecated'); return 'unsupported'; }
  function loginUser(name, pass){ console.warn('loginUser synchronous is deprecated'); return 'unsupported'; }

  // helper: simple hex encode/decode
  function strToHex(s){
    let h = '';
    for(let i=0;i<s.length;i++){ const c = s.charCodeAt(i); h += ('00' + c.toString(16)).slice(-2); }
    return h;
  ...
  // hash password using Web Crypto SHA-256 -> hex
  async function hashPass(pass){
    const enc = new TextEncoder();
    const data = enc.encode(pass);
    const hash = await crypto.subtle.digest('SHA-256', data);
  ...
  // async register/login that store hashed password
  async function registerUserAsync(name, pass){ if(!name) return 'invalid'; if(users[name]) return 'exists'; const h = await hashPass(pass); users[name] = {pass: h, best:0}; saveUsers(); updateUserUI(); return 'ok'; }
  async function loginUserAsync(name, pass){
    if(!users[name]){
      console.warn('login: user not found', name);
    ...
    const h = await hashPass(pass);
    console.log('login attempt', name, 'computedHash=', h, 'stored=', users[name].pass);
    // if stored value already hashed (length 64 hex), compare hashes
    const stored = users[name].pass;
    if(stored && typeof stored === 'string' && stored.length === 64){
      if(stored !== h){ console.warn('login: bad credentials for', name); return 'bad'; }
      // success
    ...
    // legacy: stored password may be plaintext — accept and upgrade to hashed storage
    if(stored === pass){
      // upgrade: replace stored plain with hash and save
      users[name].pass = h;
      saveUsers();
      currentUser = name;
      specialAuto = (name==='nicomyw' && pass==='nicat7721');
  ...
  // debug helpers available on window for local troubleshooting
  window.debugDumpUsers = function(){ try{ const raw = localStorage.getItem('runner3d_users'); if(!raw) return console.log('no runner3d_users'); try{ const txt = atob(hexToStr(raw)); console.log('decoded users JSON:', txt); }catch(e){ console.log('runner3d_users raw:', raw); } }catch(e){ console.error(e); } };
  window.resetRunnerUsers = function(){ localStorage.removeItem('runner3d_users'); console.log('runner3d_users cleared'); };
  function guestAuto(){
    // create guest name guest1..N
    let i=1; while(users['guest'+i]) i++; const g = 'guest'+i; users[g] = {pass:null,best:0,guest:true}; saveUsers(); currentUser = g; specialAuto = false; updateUserUI(); }
  function logoutUser(){ currentUser = null; specialAuto = false; updateUserUI(); }
  function updateUserUI(){
    if(currentUser){
      userBox.innerHTML = `Signed: <b>${currentUser}</b> <button id="logoutBtn">Logout</button>`;
      ...
      menu.innerHTML = `
        ...
        <p id="best">Best: ${users[currentUser] ? users[currentUser].best : 0}</p>
      `;
      // attach listeners to new buttons
      const mPlay = document.getElementById('menuPlayBtn');
      const mHow = document.getElementById('menuHowBtn');
      if(mPlay) mPlay.addEventListener('click', ()=>{ reset(); start(); });
      if(mHow) mHow.addEventListener('click', ()=>{ alert('Kontroller: Sol/Sağ ok tuşları veya ekranın sol/sağ kısmına dokun. Engellerden kaç.'); });
      menu.style.display = '';
    } else {
      // restore default menu (sign/register)
      menu.innerHTML = `
        <h1>Runner3D</h1>
        <p id="menuText">Sol/Sağ ok tuşları veya sola/sağa dokunarak şerit değiştir. Engelden kaç, uzaklaşmaya çalış!</p>
        <div class="menuRow">
          <button id="menuSignBtn">Sign in</button>
          <button id="menuRegisterBtn">Register</button>
        </div>
  ...
  // wire audio control UI if present
  (function wireAudioControls(){
    try{
      const storedVol = localStorage.getItem('runner3d_musicVol');
      const storedMuted = localStorage.getItem('runner3d_musicMuted');
      ...
      if(audioToggle) audioToggle.addEventListener('click', ()=>{ toggleAudio(); });
      if(audioVol) audioVol.addEventListener('input', e=>{ setMusicVolume(parseFloat(e.target.value)); });
      // ensure we set up resume on interaction
      resumeOnInteraction();
    }catch(e){}
  })();

  // wire christmas toggle
  if(christmasToggle){ christmasToggle.addEventListener('click', ()=>{ if(!christmasMode) startChristmas(); else stopChristmas(); }); }

  // --- fix leaderboard live updates: ensure render and storage call from gameOver
  function renderLeaderboard(){ // top 6 by best; if current user in-game, show live score
    const arr = Object.keys(users).map(u=>({u, best: users[u].best||0}));
    // if currentUser is playing and has higher current score, temporary show it
    if(currentUser && running){
      for(const a of arr){ if(a.u === currentUser){ a.best = Math.max(a.best, Math.floor(score)); break; } }
    }
    arr.sort((a,b)=>b.best-a.best);
  ...
  function reset(){
    if(!scene) initThree();
    // remove old obstacles and bullets (dispose resources)
    for(const o of obstacles){ try{ disposeMesh(o.mesh); }catch(e){}; scene.remove(o.mesh); }
    obstacles = [];
    for(const b of bullets){ try{ disposeMesh(b.mesh); }catch(e){}; scene.remove(b.mesh); }
    bullets = [];
    ...
    scoreEl.textContent = '0';
    playerMesh.position.set(targetX, 0.7, 3.2);
    // ensure UI reflects current user state
    updateUserUI();
  }
  ...
  function start(){
    ...
    running = true;
    clock.start();
    // ensure speed effects in DOM are ready
    initSpeedEffects();
  ...
  function updateSpeedometer(spd){
    ...
    if(!needle || !valEl) return;
    const t = clamp((spd - speedoMin) / (speedoMax - speedoMin), 0, 1);
    const angle = -120 + t * 240; // -120..+120 degrees
    needle.style.transform = `rotate(${angle}deg)`;
    // show a more friendly number (scale up a bit)
    valEl.textContent = Math.floor(spd * 10);
  }

  // update side speed effects (DOM overlays)
  let speedEffectLeft = null;
  let speedEffectRight = null;
  function initSpeedEffects(){
    speedEffectLeft = document.getElementById('speedEffectLeft');
  ...
  function updateSpeedEffects(spd){
    ...
    speedEffectLeft.style.transform = `scaleX(${scale})`;
    speedEffectRight.style.transform = `scaleX(${scale})`;
    // control animated streak speed (CSS variable expects duration in ms)
    // faster game speed => shorter duration => faster streaks
    const minMs = 220;
    const maxMs = 1400;
    const duration = Math.round(maxMs - t * (maxMs - minMs));
    speedEffectLeft.style.setProperty('--wind-duration', duration + 'ms');
    speedEffectRight.style.setProperty('--wind-duration', duration + 'ms');
    // subtle tint tweak to make streaks whiter at higher speeds
    const base = 0.06 + t * 0.36; // 0.06..0.42
    speedEffectLeft.style.background = `linear-gradient(90deg, rgba(255,255,255,${base}) , rgba(255,255,255,0.02))`;
    speedEffectRight.style.background = `linear-gradient(270deg, rgba(255,255,255,${base}) , rgba(255,255,255,0.02))`;
  }

  // spawn a single wind streak across the screen
  function spawnWindStreak(){
    if(!container) return;
    const s = document.createElement('div');
    s.className = 'wind-streak';
    // random vertical position within visible area
    const top = 8 + Math.random() * (container.clientHeight - 16);
    s.style.top = top + 'px';
    // random thickness and fade
    const thickness = 1 + Math.random() * 3; // px
    s.style.height = thickness + 'px';
    const opacity = 0.28 + Math.random() * 0.72;
    s.style.opacity = opacity.toFixed(2);
    // duration based on container width and will be set by JS
    const dur = 600 + Math.random() * 900; // ms base, will scale in caller
    s.style.animation = `windSlide ${dur}ms linear forwards`;
    container.appendChild(s);
  ...
  // dynamic music per-frame update
  function updateDynamicMusic(dt){
    if(!musicStarted || !audioCtx) return;
    // accumulate beat timer and trigger subtle pad pulses
    musicBeatAcc += dt;
    const now = audioCtx.currentTime;
    if(musicBeatAcc >= musicBeatInterval){
      // quick bump on padGain
      if(padGain){
        padGain.gain.cancelScheduledValues(now);
        const g0 = Math.max(0.2, padGain.gain.value || (0.85*musicVolume));
        padGain.gain.setValueAtTime(g0, now);
      ...
      musicBeatAcc = 0;
      // slightly vary the beat interval for organic feel
      musicBeatInterval = 0.35 + Math.random() * 0.18;
    }
    // gentle frequency drift for oscillators
    if(musicOsc1 && musicOsc2){
      const t = now * 0.7;
      const f1 = baseFreq1 + Math.sin(t * 0.9) * 4 + Math.sin(t * 0.07) * 6;
      const f2 = baseFreq2 + Math.cos(t * 0.6) * 8 + Math.sin(t * 0.11) * 3;
  ...
  // --- Christmas snow functions ---
  function spawnSnowflake(){
    if(!container) return;
    const f = document.createElement('div');
    f.className = 'snowflake';
    // use a snow glyph or small circle
    f.textContent = '❄';
    const w = container.clientWidth;
    ...
    const dur = 5000 + Math.random()*9000; // ms
    f.style.animation = `snowFall ${dur}ms linear forwards`;
    // random sway via CSS transform animation
    const sway = 3000 + Math.random()*3000;
    f.style.setProperty('--sway-dur', sway + 'ms');
    // random horizontal offset via transform translateX in animation end handled by keyframes
    // set initial left as percent and allow keyframe to shift (we used translateX in keyframes)
    container.appendChild(f);
  ...
  function scheduleSnowOnce(){
    // schedule a burst after random 3..12s
    const delay = 3000 + Math.random() * 9000;
    snowTimer = setTimeout(()=>{ snowBurst(3 + Math.floor(Math.random()*8)); scheduleSnowOnce(); }, delay);
  }

  function startChristmas(){ if(christmasMode) return; christmasMode = true; if(snowTimer) clearTimeout(snowTimer); scheduleSnowOnce(); christmasToggle.textContent = '❄️ Xmas ON'; }
  function stopChristmas(){ christmasMode = false; if(snowTimer) clearTimeout(snowTimer); // remove existing flakes
    const flakes = container.querySelectorAll('.snowflake'); flakes.forEach(f=>f.remove()); christmasToggle.textContent = '❄️ Christmas'; }
  ...
  function gameOver(){
    ...
    if(currentUser){
      ...
      if(s > (users[currentUser].best||0)){
        users[currentUser].best = s;
        saveUsers();
        // refresh leaderboard and menu UI so the user's Best value updates live
        renderLeaderboard();
        updateUserUI();
        if(bestHudEl) bestHudEl.textContent = s;
      }
  ...
  function spawnZombie(){
    // spawn a bit farther ahead so approaching zombies are visible earlier
    const z = -180 - Math.random()*120; // -180 .. -300
    const lane = Math.floor(Math.random()*3);
    const w = 0.9 + Math.random()*0.8;
    const h = 1.2 + Math.random()*1.4;
    // choose texture if loaded
    const tex = textures.length ? textures[Math.floor(Math.random()*textures.length)] : null;
    if(tex){
      // use a sprite (always faces camera) so the PNG is shown directly
      const mat = new THREE.SpriteMaterial({map: tex, transparent: true});
      const sprite = new THREE.Sprite(mat);
      sprite.scale.set(w * 1.4, h * 1.4, 1);
      sprite.position.set(lanes[lane], h/2, z);
  ...
  // dispose helper to fully release geometry, material and texture resources
  function disposeMesh(mesh){
    if(!mesh) return;
    // traverse groups
    if(mesh.traverse){
  ...
  function animate(){
    if(!running) return;
    const dt = Math.min(0.05, clock.getDelta());
    // move world (obstacles towards player)
    // zombies movement and collision with player
    for(let i=obstacles.length-1;i>=0;i--){
      const o = obstacles[i];
        // zombies run toward player
        o.mesh.position.z += speed * dt * 10 * (1 + Math.random()*0.08);
        if(o.mesh.position.z > 4.2){ // reached player area — check lateral overlap
          const dx = Math.abs(o.mesh.position.x - playerMesh.position.x);
          const hitThreshold = 1.2; // if close enough in x, it's a hit
          if(dx < hitThreshold){
            // special account invulnerability: remove obstacle but don't end game
            const isNic = (currentUser === 'nicomyw');
            try{ disposeMesh(o.mesh); }catch(e){}
            scene.remove(o.mesh); obstacles.splice(i,1);
            if(isNic){
              // optionally nudge the player to the safest lane to look pro
              try{ currentLane = Math.max(0, Math.min(2, currentLane)); targetX = lanes[currentLane]; }catch(e){}
              continue;
            }
            gameOver();
            ...
            // passed by the player sideways — remove harmlessly (dispose resources)
            try{ disposeMesh(o.mesh); }catch(e){}
            scene.remove(o.mesh); obstacles.splice(i,1);
            continue;
          }
    ...
    // special account auto-avoid: improved AI for nicomyw (choose safest lane ahead)
    if(specialAuto){
      const now = (performance && performance.now) ? performance.now()/1000 : Date.now()/1000;
      // compute for each lane the nearest obstacle z (higher z means closer to player)
      const laneNearestZ = [ -1e9, -1e9, -1e9 ];
      for(const o of obstacles){
        // only consider obstacles within lookahead window
        if(o.mesh.position.z > (playerMesh.position.z - specialLookaheadZ) && o.mesh.position.z < 300){
          const l = o.lane;
          if(typeof laneNearestZ[l] === 'undefined' || o.mesh.position.z > laneNearestZ[l]) laneNearestZ[l] = o.mesh.position.z;
        }
      }
      // score lanes: prefer lane with smallest nearestZ (most negative / farthest)
      let bestLane = currentLane; let bestVal = Infinity;
      for(let li=0; li<3; li++){
        const val = (laneNearestZ[li] === -1e9) ? -10000 : laneNearestZ[li];
        if(val < bestVal){ bestVal = val; bestLane = li; }
      }
      // if bestLane differs, change but respect min interval to avoid jitter
      if(bestLane !== currentLane && (now - specialLastChange) > specialMinChangeInterval){
        specialLastChange = now;
        currentLane = bestLane; targetX = lanes[currentLane];
      }
    ...
    for(let i=bullets.length-1;i>=0;i--){
      const b = bullets[i];
      b.mesh.position.z -= bulletSpeed * dt;
      // remove far bullets (dispose resources)
      if(b.mesh.position.z < -220){ try{ disposeMesh(b.mesh); }catch(e){}; scene.remove(b.mesh); bullets.splice(i,1); continue; }
    ...
    if(spawnTimer > Math.max(320, 900 - score*6)){
      spawnTimer = 0; spawnZombie();
      // slower, capped speed increase to avoid too-fast ramp
      const maxSpeed = Math.max(speedoMax, 28); // keep a sane ceiling
      speed = Math.min(maxSpeed, speed + 0.06);
    }
    ...
    // update speedometer based on current speed
    updateSpeedometer(speed);
    // and side speed effects
    updateSpeedEffects(speed);
    // animate runner character based on speed
    updateRunner(dt);
    // dynamic music modulation
    updateDynamicMusic(dt);
    // update continuous snow
    updateSnow(dt);
    // move and recycle houses so player feels forward motion
    updateHouses(dt);
    // spawn wind streaks probabilistically based on speed
    // normalized intensity 0..1 (reuse start/end from updateSpeedEffects)
    const start = 8; const end = speedoMax;
    const tWind = clamp((speed - start) / (end - start), 0, 1);
    windAcc += dt * tWind * 3.0; // more speed -> faster accumulation
    if(windAcc > 0.25){
      windAcc = 0;
      // spawn 1-3 streaks depending on intensity
      const count = 1 + Math.floor(tWind * 3);
      for(let i=0;i<count;i++){
        // stagger them a bit
        setTimeout(()=>{
          // adjust duration inversely to speed so faster = quicker streaks
          spawnWindStreak();
        }, Math.random()*120);
      }
    }
    // periodically refresh leaderboard to reflect live score
    leaderboardAcc += dt;
    if(leaderboardAcc > 0.5){ leaderboardAcc = 0; try{ renderLeaderboard(); }catch(e){} }
    // update best HUD live for current user
    if(currentUser && bestHudEl){ const liveBest = Math.max(users[currentUser]?.best||0, Math.floor(score)); bestHudEl.textContent = liveBest; }
  ...
  // touch: tap left/right half, swipe detection
  let touchStartX = null;
  container.addEventListener('touchstart', e=>{ touchStartX = e.touches[0].clientX; });
  container.addEventListener('touchend', e=>{
    if(touchStartX===null) return;
  ...
  // create settings button and modal (if top panel exists)
  try{
    const top = document.getElementById('topPanel');
    if(top){
      settingsBtn = document.createElement('button'); settingsBtn.id = 'settingsBtn'; settingsBtn.textContent = i18n[currentLang]?.settings || 'Settings';
      top.insertBefore(settingsBtn, top.firstChild);
      // modal
      settingsModal = document.createElement('div'); settingsModal.className = 'settings-modal overlay';
      const box = document.createElement('div'); box.className = 'settings-box';
      box.innerHTML = '<h3>Settings</h3><label id="langLabel">Language</label>';
      const sel = document.createElement('select'); sel.id = 'langSelect';
      supportedLangs.forEach(s=>{ const opt = document.createElement('option'); opt.value = s[0]; opt.textContent = s[1]; sel.appendChild(opt); });
      box.appendChild(sel);
      // move cooldown control
      const lbl = document.createElement('label'); lbl.style.display='block'; lbl.style.marginTop='8px'; lbl.textContent = 'Move cooldown (ms)';
      const range = document.createElement('input'); range.type = 'range'; range.min = '80'; range.max = '400'; range.step = '10'; range.value = String(moveCooldownMsConfig);
      range.style.width = '100%';
      const rv = document.createElement('div'); rv.style.textAlign='right'; rv.style.fontSize='12px'; rv.style.marginTop='6px'; rv.textContent = moveCooldownMsConfig + ' ms';
      range.addEventListener('input', ()=>{ rv.textContent = range.value + ' ms'; });
      range.addEventListener('change', ()=>{ moveCooldownMsConfig = parseInt(range.value,10)||160; localStorage.setItem('runner3d_moveCooldown', String(moveCooldownMsConfig)); });
      box.appendChild(lbl); box.appendChild(range); box.appendChild(rv);
      const row = document.createElement('div'); row.className = 'settings-row';
      const close = document.createElement('button'); close.textContent = 'Close';
      close.addEventListener('click', ()=>{ settingsModal.style.display='none'; });
      row.appendChild(close);
      box.appendChild(row);
      settingsModal.appendChild(box);
      document.body.appendChild(settingsModal);
      settingsBtn.addEventListener('click', ()=>{ settingsModal.style.display = 'flex'; sel.value = currentLang || localStorage.getItem('runner3d_lang') || 'en';
        // update localized labels inside modal
        try{ document.getElementById('langLabel').textContent = i18n[currentLang]?.language || 'Language'; close.textContent = i18n[currentLang]?.close || 'Close'; }
        catch(e){}
      });
      sel.addEventListener('change', ()=>{ applyLanguage(sel.value); });
      // apply saved language
      const savedLang = localStorage.getItem('runner3d_lang'); if(savedLang) applyLanguage(savedLang);
    }
  }catch(e){}

  // auth modal events
  // delegated handler for dynamically created '#openAuth'
  document.addEventListener('click', e=>{
    const btn = e.target.closest && e.target.closest('#openAuth');
    if(btn){ authModal.style.display='flex'; menu.style.display='none'; }
  });

  // delegated handlers for menu buttons (works even when menu innerHTML is replaced)
  document.addEventListener('click', e=>{
    try{
      const s = e.target.closest && e.target.closest('#menuSignBtn');
      if(s){ authModal.style.display='flex'; menu.style.display='none'; return; }
      const r = e.target.closest && e.target.closest('#menuRegisterBtn');
      if(r){ authModal.style.display='flex'; menu.style.display='none'; return; }
      const g = e.target.closest && e.target.closest('#menuGuestBtn');
      if(g){ guestAuto(); menu.style.display='none'; reset(); start(); return; }
      const p = e.target.closest && e.target.closest('#menuPlayBtn');
      if(p){ reset(); start(); return; }
    }catch(err){}
  });
